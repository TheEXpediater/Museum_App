...
def rasterize_position_atlas(
    ...
    # backend="egl": the default backend on Linux is X11/GLX (moderngl/glcontext's
    # default_backend()), which needs a real X server and fails on this headless VPS with
    # "XOpenDisplay: cannot open display" - proven by a real failed bake here, not a guess. EGL
    # can create a "surfaceless" context against Mesa's software renderer (llvmpipe, verified
    # working via GL_RENDERER) without any display server, which is what a Docker container needs.
...
def positions_to_colors(model, scene_code, positions_texture, texture_resolution):
    # Upstream bug fix: the official implementation builds `positions` on CPU and never moves it
    # to the model's device, which crashes with a CUDA/CPU device mismatch inside
    # query_triplane's grid_sampler_2d as soon as the model runs on CUDA (reproduced here with a
    # real GPU run - see infer.py's bake-texture fallback log). Only the device placement is
    # changed; the math is untouched.
